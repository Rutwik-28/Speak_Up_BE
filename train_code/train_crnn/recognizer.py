...
def recognize_downline(img,crnn_model=model):
    ...
    preds_size = Variable( torch.IntTensor( [preds.size( 0 )] ) ) # prediction size
    raw_pred = converter.decode( preds.data, preds_size.data, raw=True ) # raw prediction
    sim_pred = converter.decode( preds.data, preds_size.data, raw=False ) # simple prediction
