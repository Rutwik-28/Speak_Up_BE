...
def get_det_boxes(image,display = True, expand = True):
    ...
    with torch.no_grad():
        ...
        cls_prob = F.softmax(cls, dim=-1).cpu().numpy() # when output more than one or two nodes
        regr = regr.cpu().numpy()
        # Red colour boxes are generated
        anchor = gen_anchor((int(h / 16), int(w / 16)), 16)
        bbox = bbox_transfor_inv(anchor, regr)
        bbox = clip_box(bbox, [h, w])
        # print(bbox.shape)

        # Unwanted predicted result whose probablity is low is removed
